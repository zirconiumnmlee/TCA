...
INDEX_NAME = "2wiki_test"


def load_2wiki_data(input_dir: str) -> List[Dict[str, Any]]:
    """加载 2wiki 数据集并转换为可索引格式"""
    file_path = os.path.join(input_dir, "2wiki_test_200.json")
...
def main(args):
    config = Config(dataset="2wiki_test")
    input_dir = config.input_corpus_dir

    print(f"📂 Loading 2wiki_test data from: {input_dir}")
    documents = load_2wiki_data(input_dir)
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Build Elasticsearch sparse index for 2wiki_test")
