...
INDEX_NAME = "musique_test"


def load_musique_data(input_dir: str) -> List[Dict[str, Any]]:
    """加载 musique 数据集并转换为可索引格式"""
    file_path = os.path.join(input_dir, "musique_test_200.json")
...
def main(args):
    config = Config(dataset="musique_test")
    input_dir = config.input_corpus_dir

    print(f"📂 Loading musique data from: {input_dir}")
    documents = load_musique_data(input_dir)
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Build Elasticsearch sparse index for musique")
